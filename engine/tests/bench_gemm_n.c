 ...
 *              (coli_vk_bench_gemm4) -- host submission-to-fence wall time,
 *              amortizing one submit/fence over reps; not GPU timestamp queries
...
int main(int argc,char**argv){
  ...
  unsigned seed=7; int fail=0, unavailable=0;
  for (size_t si=0; si<sizeof shapes/sizeof shapes[0]; si++) {
    ...
    double best=INFINITY; int valid=0;
    for(int k=0;k<3;k++) {
      double s=coli_vk_bench_gemm4(v,h,&a,Yg,reps);
      if(isfinite(s) && s>0) { valid++; if(s<best)best=s; }
    }
    ...
    printf("  %-22s ", shapes[si].name);
    if(valid) printf("device %9.1f us (%6.1f TFLOPS)", dev_us, flop/dev_us/1e6);
    else { printf("device N/A (no valid timing)"); unavailable++; }
    printf("   per-call median %9.1f us min %9.1f (%5.1f TFLOPS)", call_us, call_min, flop/call_us/1e6);
    if(valid) printf("   round-trip %+8.1f us", call_us-dev_us);
    else printf("   round-trip N/A");
    printf("   device samples %d/3   rel=%.1e %s\n", valid, rel, ok?"ok":"BAD");
    if(!ok) fail=1;
    free(W);free(X);free(Yg);free(Yc);free(a.q);free(a.scale);free(a.sum);free(t);
  }
  if(unavailable) printf("NOTE: device timing unavailable for %d shapes; correctness uses per-call results only\n", unavailable);
  printf(fail?"FAIL\n":"PASS\n"); return fail;
}
